...
**Figura:** Esquema conceptual dun modelo en estrela. 
Fonte: documentación oficial de Power BI.
...
Pasar `DimDate` a táboa de datas (recomendado):

1. Vai á vista `Modelo` (ou `Datos`) e selecciona a táboa `DimDate`.
2. Na cinta superior, abre `Herramientas de tabla`.
3. Preme `Marcar como tabla de fechas`.
4. Escolle a columna `DimDate[Date]` como referencia.
5. Confirma os cambios.

Comprobación rápida:

1. revisa que non haxa valores nulos en `DimDate[Date]`
2. comproba que non existan datas duplicadas
3. valida despois unha medida temporal (por exemplo `Sales PY` con `SAMEPERIODLASTYEAR`)
...
3. Fai clic dereito sobre `Fiscal Year` e escolle `Crear jerarquía`.
...
2. Fai clic dereito en `Category` e escolle `Crear jerarquía`.
...
2. Arrastra `Fiscal Date Hierarchy` a `Eje X`.
3. Se xa creaches a medida, arrastra `Total Sales` a `Eje Y`.
4. Se aínda non creaches a medida, arrastra `FactSales[Sales Amount]` a `Eje Y` e deixa a agregación en `Suma`.
...
- ordenar campos temporais (por exemplo, `Month` ordenado por `MonthKey`)
