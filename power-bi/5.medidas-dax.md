...
- `TOTALYTD`  
  ...
  Exemplo: `CALCULATE([Total Sales], DATESYTD(DimDate[Date], "06/30"))`
...
2. Valida a medida nunha matriz con este procedemento:
   1. Nunha páxina do informe, insire o visual `Matriz` desde o panel de visualizacións.
...
2. Comproba os resultados primeiro por mes e despois por totais.

   Para validar ben `Sales PY`, usa unha matriz:
   ...
   4. Exemplo: se estás en `FY2020 - Jul`, o valor de `Sales PY` debería cadrar co `Total Sales` de `FY2019 - Jul`.
   5. No primeiro ano dispoñible do dataset, é normal ver `BLANK` en `Sales PY` porque non hai ano previo.
   6. Revisa tamén o subtotal de `Fiscal Year`: coa fórmula actual debería sumar correctamente os meses equivalentes do ano anterior.
...
## 9. Checklist final do documento

Antes de pasar ao seguinte bloque, comproba:

- `Total Sales`, `Total Cost`, `Margin`, `% Margin` creadas
- polo menos unha medida con `CALCULATE`
- polo menos unha medida temporal (`Sales YTD`)
- validación básica en matriz por ano/categoría
- formatos aplicados correctamente (moeda e porcentaxe)
...
Coas medidas base xa listas, o seguinte paso natural será centrarse en **visualizacións e deseño de informe**, aproveitando estas medidas para construír gráficos e paneis máis útiles.
